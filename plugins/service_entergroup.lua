...
local function description_rules(msg, user_name)
...
local function run(msg, matches)
  ...
  if matches[1] == "chat_add_user" then
    if not msg.action.user.username then
      user_name = string.gsub(msg.action.user.print_name, "_", " ")
    else 
      user_name = "@"..msg.action.user.username
    end
    chat_new_user(msg)
    description_rules(msg, user_name)
